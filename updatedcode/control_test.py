...
class SumoIntersection:
    ...
    def chooseMode(self, junctions):
        ...
        averageQueueLength = traci.edge.getLastStepHaltingNumber('R43') + traci.edge.getLastStepHaltingNumber(
                        'R49') + traci.edge.getLastStepHaltingNumber('R46') + traci.edge.getLastStepHaltingNumber('R48')
    ...
    def getState(self):
        ...
        junctionPosition = traci.junction.getPosition('N12')[0]
        vehicles_road1 = traci.edge.getLastStepVehicleIDs('R43')
        vehicles_road2 = traci.edge.getLastStepVehicleIDs('R49')
        vehicles_road3 = traci.edge.getLastStepVehicleIDs('R46')
        vehicles_road4 = traci.edge.getLastStepVehicleIDs('R48')
        ...
        junctionPosition = traci.junction.getPosition('N12')[1]
        for v in vehicles_road3:
            ind = int(
...
if __name__ == '__main__':
    ...
    for e in range(episodes):
        ...
        sumoProcess = subprocess.Popen([sumoBinary, "-c", "map/gwalior.sumocfg", "--tripinfo-output",
                                    "tripinfo.xml","--quit-on-end","false","--start","false", "--remote-port",str(PORT)], stdout=sys.stdout, stderr=sys.stderr)
        traci.init(PORT)
        junctions = traci.trafficlight.getIDList()
        traci.trafficlight.setPhase("N12", 0)
        traci.trafficlight.setPhaseDuration("N12", 200)
        turn = 0
        ...
        while traci.simulation.getMinExpectedNumber() > 0 and stepz < 10000:
            ...
            if(action == 1):
                ...
                for i in range(5):
                    stepz += 1
                    traci.trafficlight.setPhase("N12", turn)
